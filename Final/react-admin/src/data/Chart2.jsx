...
import { HistoryChasing } from "./HistoryChasing"

const Chart2 = ({room_id, callbackSetSignIn, timedelay, apiInformationTag}) => {
    const dict_filter = {"1D": 1, "1W": 2, "1M": 3, "3M": 4, "1Y": 5}
    ...
    const url = {
        0: `http://${backend_host}/api/v1.1/monitor/data?room_id=${room_id}&filter=${numberOfData}&node_id=${nodeIdFilter}`,
        1: `http://${backend_host}/api/v1.1/monitor/data/history?room_id=${room_id}&node_id=${nodeIdFilter}&time_start=${unixTimestampStart}&time_end=${unixTimestampEnd}&option=${optionChartData}`,
        2: `http://${backend_host}/api/v1.1/monitor/data?room_id=${room_id}&filter=${1}&node_id=${nodeIdFilter}`,
    }    
    ...
    useEffect(() => {
        if(timedelay === 0)
        {
            verify_and_get_data(get_chart_data, callbackSetSignIn, host, url[urlOption]);
        }
        else
        {
            ...
            >
                {/* Container of filterParameter */}
                    ...
                    >
                        ...
                        <Box>
                            <Header title={"Optional: "} fontSize={"20px"} />
                        </Box>
                        <HistoryChasing
                            setOptionChartData = {setOptionChartData}
                            setOptionData = {setOptionData}
                            setUnixTimestampStart = {setUnixTimestampStart}
                            setUnixTimestampEnd = {setUnixTimestampEnd}
